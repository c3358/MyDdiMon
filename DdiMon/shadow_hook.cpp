...
_Use_decl_annotations_ EXTERN_C NTSTATUS ShEnableHooks()// Enables page shadowing for all hooks
{
  ...
  return UtilForEachProcessor([](void* context) {
...
_Use_decl_annotations_ NTSTATUS ShEnablePageShadowing(EptData* ept_data, const SharedShadowHookData* shared_sh_data)// Enables page shadowing for all hooks
{
...
_Use_decl_annotations_ EXTERN_C static SIZE_T ShpGetInstructionSize(void* address)
...
{
  const auto found = std::find_if(shared_sh_data->hooks.cbegin(), shared_sh_data->hooks.cend(), [address](const auto& info) {
